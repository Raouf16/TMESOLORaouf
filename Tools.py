...
class PlayerStateDecorator : 
	...
	def replacementPasseur(self):
		for(idt, idp) in self.state.player_state:
			if(idt == self.id_team):
				if(idp != self.id_player):
					return SoccerAction((self.state.player_state(idt, idp).position - self.position_joueur) / 2, Vector2D(0,0))
